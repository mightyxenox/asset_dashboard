...
const BuySellPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [resCoins, resWallets, userC, userW] = await Promise.all([
        ...
        setUserCoins((userC.coins || []).map(c => typeof c === 'string' ? c : c.name));
        setUserWallets((userW.wallets || []).map(w => typeof w === 'string' ? w : w.name));
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    const availableCoins = mode === 'buy' ? allCoins : userCoins;
    const availableWallets = mode === 'buy' ? allWallets : userWallets;
